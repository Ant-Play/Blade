...
void UATPCCameraLocationObject::Tick(float DeltaTime)
{
	...
	{
		camera.SetRelativeLocation(FMath::VInterpConstantTo(camera.GetRelativeLocation(), AttachCameraRelativeLocation, DeltaTime, AttachCameraInterpSpeed));
	...
	if (cameraMode.bEnableLocationSettings)
	{
		...
		TargetOffset.Interpolate(true, DeltaTime);
		UpdateSocketOffsetTarget();
		OldSocketOffset = SocketOffset;
		SocketOffset.Interpolate(true, DeltaTime);
	}
	...
	else if (cameraMode.bEnableRotationSettings)
	{
		...
		UpdateOffsetCameraRotationTime(DeltaTime);

		UpdateViewRotationToActorRotationTime(DeltaTime);

		ValidateCameraView(true, DeltaTime);
	}

	LastTargetRotation = GetTargetRotation();

	UpdateDesireArmLocation(cameraMode.LocationSettings.bDoCollisionTest, cameraMode.LocationSettings.bEnableCameraLocationLag, cameraMode.LocationSettings.bEnableCameraRotationLag, DeltaTime);
...
void UATPCCameraLocationObject::OnEnterCameraMode(bool bWithInterpolation)
{
	...
	SocketOffset.InterpolationSpeed = locationSettings.SocketOffsetInterpolation;
	TargetOffset.InterpolationSpeed = locationSettings.TargetOffsetInterpolation;
...
{
	const APawn* owningPawn = GetOwningPawn();
	return owningPawn != nullptr ? owningPawn->GetViewRotation() : GetDesiredRotation();
}
...
void UATPCCameraLocationObject::UpdateDesireArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
	auto& camera = GetCamera();
	...
	UE_VLOG_LOCATION(this, LogTPC, Log, armOrigin, 15, FColor::Orange, TEXT("Arm origin"));
	UE_VLOG_LOCATION(this, LogTPC, Log, armOriginWithLag, 15, FColor::Green, TEXT("Arm origin lag"));
	...
	if (bDoTrace)
	{
		...
		UE_VLOG_ARROW(this, LogTPC, Log, traceStart, traceEndOldDist, FColor::Orange, TEXT("Trace old dist"));
		if (hitResultOldDist.bBlockingHit)
		{
			UE_VLOG_LOCATION(this, LogTPC, Log, hitResultOldDist.Location, locationSettings.ProbeSize, FColor::Orange, TEXT("Blocking hit trace old dist"));
		}
		...
		UE_VLOG_ARROW(this, LogTPC, Log, traceStart, traceEnd, FColor::Green, TEXT("Trace dist"));
		if (hitResult.bBlockingHit)
		{
			UE_VLOG_LOCATION(this, LogTPC, Log, hitResult.Location, locationSettings.ProbeSize, FColor::Green, TEXT("Blocking hit trace"));
		}
		...
		if (locationSettings.bDoMovementCollisionTest && hitResult.bBlockingHit && hitResult.Component.IsValid() && hitResult.Component->GetCollisionResponseToChannel(locationSettings.MovementCollisionTestCollisionChannel) == ECR_Overlap)
		{
		...
		resultLoc = BlendLocations(desiredLoc, armOriginWithLag, hitResult.bBlockingHit, hitResult.Location, DeltaTime);
...
{
	static const FRotator defaultRotator = FRotator();
	return OverrideCameraRotation.Get(defaultRotator);
}


void UATPCCameraLocationObject::AttachCameraToComponent(USceneComponent* ParentComponent, FVector RelativeLocation, FName SocketName, bool bWithInterp, float InterpSpeed)
{
	auto& camera = GetCamera();
	...
	camera.AttachToComponent(ParentComponent, attachmentRules, SocketName);

	bAttachCameraWithInterp = bWithInterp;
	AttachCameraRelativeLocation = RelativeLocation;
...
float UATPCCameraLocationObject::GetCameraDistanceModifier() const
{
...
{
	return CameraDistance.TargetDistance + GetCameraDistanceModifier();
}
...
{
	return const_cast<FATPCRotationOffsetSettings&>(std::as_const(*this).GetCurrentRotationOffsetSettings());
}
...
void UATPCCameraLocationObject::UpdateOffsetCameraRotationTime(float DeltaSeconds)
{
	...
	if (bEnableRotationOffset)
	{
		...
		if (owningPawn != nullptr && owningPawn->Controller != nullptr)
		{
			...
			if (FMath::IsNearlyEqual(LastTargetRotation.Pitch, targetRotation.Pitch) && (!rotationOffsetSettings.bActivateOnlyIfMovement || owningPawn->GetVelocity().SizeSquared() > FMath::Square(rotationOffsetSettings.MinMovementSpeedForActivate)))
			{
...
void UATPCCameraLocationObject::UpdateOffsetCameraRotation(bool bWithInterp, float DeltaSeconds)
{
	...
	if (owningPawn != nullptr && owningPawn->Controller != nullptr)
	{
		...
		newViewRotation.Pitch = bWithInterp ? UATPCFunctionLibrary::InterpRotator(FRotator(newViewRotation.Pitch, 0.f, 0.f), FRotator(pitchTarget, 0.f, 0.f), DeltaSeconds, rotationOffsetSettings.RotationInterpolation).Pitch : pitchTarget;
...
void UATPCCameraLocationObject::UpdateViewRotationToActorRotationTime(float DeltaSeconds)
{
	...
	if (rotationSettings.bEnableViewRotationToActorRotation && owningPawn != nullptr && owningPawn->Controller != nullptr)
	{
		...
		if (actorToViewRotDelta < cameraToActorRotationSettings.MaxDeltaForChange && FMath::IsNearlyEqual(LastTargetRotation.Yaw, viewRotationYaw, 0.01f) && owningPawn->GetVelocity().SizeSquared() > KINDA_SMALL_NUMBER)
		{
