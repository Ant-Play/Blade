...
void UATPCCameraFollowTerrainObject::Tick(float DeltaTime)
{
	...
	if (GetCamera().GetCurrentCameraMode()->CameraModeSettings.bEnableFollowTerrainSettings)
	{
		...
		UpdateRotationAndLocationOffset(true, DeltaTime);
	}
...
void UATPCCameraFollowTerrainObject::OnEnterCameraMode(bool bWithInterpolation)
{
	...
	{
		TerrainSocketOffset.InterpolationSpeed = cameraMode.FollowTerrainSettings.SocketOffsetInterpolation;
	}
	else
	{
