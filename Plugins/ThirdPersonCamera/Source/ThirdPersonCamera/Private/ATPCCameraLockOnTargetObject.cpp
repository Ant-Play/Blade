...
#include "ATPCLockOnTargetLocComponent.h"
...
void UATPCCameraLockOnTargetObject::Tick(float DeltaSeconds)
{
	...
	if (TargetActor.IsValid() && locationObject != nullptr && (lockOnTargetSettings.RotateCameraByPitch || lockOnTargetSettings.RotateCameraByYaw))
	{
		...
		const FRotator viewRotationDelta = locationObject->GetLastTickTargetRotationDelta();

		// If has player input
		if (!FMath::IsNearlyEqual(viewRotationDelta.Yaw, 0.f, minInputDelta) || !FMath::IsNearlyEqual(viewRotationDelta.Pitch, 0.f, minInputDelta))
		{
			if (lockOnTargetSettings.bResetTargetAfterPlayerInput)
			{
				if (AccumulatedPlayerInput.IsSet())
				{
					AccumulatedPlayerInput->Pitch += viewRotationDelta.Pitch;
					AccumulatedPlayerInput->Yaw += viewRotationDelta.Yaw;
				}
				else
				{
					AccumulatedPlayerInput = viewRotationDelta;
				}
		...
		if (lockOnTargetSettings.bResetTargetAfterPlayerInput && AccumulatedPlayerInput.IsSet() && (FMath::Abs(AccumulatedPlayerInput->Yaw) > FMath::Abs(lockOnTargetSettings.RequiredPlayerInputForResetTarget.Yaw) || FMath::Abs(AccumulatedPlayerInput->Pitch) > FMath::Abs(lockOnTargetSettings.RequiredPlayerInputForResetTarget.Pitch)))
		{
...
float UATPCCameraLockOnTargetObject::GetDistanceSquaredToActor(AActor* Actor) const
{
...
FVector UATPCCameraLockOnTargetObject::GetTargetLocation(AActor* Actor) const
{
	if (Actor != nullptr)
	{
		auto locationComponent = Actor->FindComponentByClass<UATPCLockOnTargetLocComponent>();
...
bool UATPCCameraLockOnTargetObject::IsValidDistanceToActor(AActor* Actor) const
{
	...
	{
		auto& lockOnTargetSettings = GetLockOnTargetSettings();
		return !lockOnTargetSettings.bUseMaxLockDistance || GetDistanceSquaredToActor(Actor) <= lockOnTargetSettings.MaxLockDistance * lockOnTargetSettings.MaxLockDistance;
	}
	return false;
}
...
{
	return const_cast<FATPCLockOnTargetSettings&>(std::as_const(*this).GetLockOnTargetSettings());
}
...
void UATPCCameraLockOnTargetObject::PrintDebug()
{
	...
	if (TargetActor.IsValid())
	{
		...
		GEngine->AddOnScreenDebugMessage(-1, 0.f, color, FString::Printf(TEXT("Distance to target actor: %.2f"), FMath::Sqrt(GetDistanceSquaredToActor(TargetActor.Get()))));
		...
		if (lockOnTargetSettings.bUseDesiredConeRotation)
		{
			...
			::DrawDebugLine(GetWorld(), lineStart, centerLineEnd, coneColor, false, -1.f, 0.f, 1.5f);
			...
			::DrawDebugLine(GetWorld(), lineStart, leftLineEnd, coneColor, false, -1.f, 0.f, lineThickness);
			...
			::DrawDebugLine(GetWorld(), lineStart, rightLineEnd, coneColor, false, -1.f, 0.f, lineThickness);
		}
	}
	else
	{
