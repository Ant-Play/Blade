...
{
	return InterpImpl(Current, Target, DeltaTime, InterpSpeed, bResetSpeedIfResultEqualToTarget, [](auto& LocalCurrent, auto& LocalTarget, auto LocalDeltaTime, auto LocalInterpSpeed) { return FMath::FInterpTo(LocalCurrent, LocalTarget, LocalDeltaTime, LocalInterpSpeed); }, [](auto& LocalCurrent, auto& LocalTarget, auto LocalDeltaTime, auto LocalInterpSpeed) { return FMath::FInterpConstantTo(LocalCurrent, LocalTarget, LocalDeltaTime, LocalInterpSpeed); }, [&](const float LocalResult, const float LocalTarget) { return FMath::IsNearlyEqual(LocalResult, LocalTarget, InterpSpeed.ResetInterpSpeedTolerance); });
}
...
bool UATPCFunctionLibrary::ReplaceSpringArmToATPCComponent(AActor* Actor)
{
	...
	if (Actor != nullptr)
	{
		if (auto springArm = Actor->FindComponentByClass<USpringArmComponent>())
		{
			...
			{
				UE_LOG(LogTPC, Warning, TEXT("Try ReplaceSpringArmToATPCComponent but atpc component not found for actor: %s"), *Actor->GetName());
			}
		}
		else
		{
			UE_LOG(LogTPC, Warning, TEXT("Try ReplaceSpringArmToATPCComponent but spring arm not found for actor: %s"), *Actor->GetName());
		}
	}
	else
	{
		UE_LOG(LogTPC, Warning, TEXT("Try ReplaceSpringArmToATPCComponent for null actor!"));
