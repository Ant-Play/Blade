...
void UATPCCameraComponent::ValidateComponent(bool bWithInterpolation)
{
...
{
	return const_cast<UATPCCameraModeDataAsset*>(std::as_const(*this).GetCurrentCameraMode());
}
...
void UATPCCameraComponent::AddCameraMode(UATPCCameraModeDataAsset* CameraMode)
{
	...
	{
		auto instanceCameraMode = DuplicateObject(CameraMode, this);
		SortedCameraModes.Add(instanceCameraMode->CameraModeTag, instanceCameraMode);
	}
}
...
UATPCCameraModeDataAsset* UATPCCameraComponent::GetCameraMode(FGameplayTag CameraModeName)
{
	return const_cast<UATPCCameraModeDataAsset*>(std::as_const(*this).GetCameraMode(CameraModeName));
}
...
void UATPCCameraComponent::SetCameraDistance(float NewDistance, bool bInterpolation)
{
	if (LocationObject != nullptr)
	{
		LocationObject->SetCameraDistance(NewDistance, bInterpolation);
	}
}
...
void UATPCCameraComponent::CheckCameraModesErrors()
{
...
void UATPCCameraComponent::PutLog(const FString& Message, float MessageTime /*= 5.f*/)
{
	...
	UE_LOG(LogTPC, Warning, TEXT("%s"), *Message);
}
