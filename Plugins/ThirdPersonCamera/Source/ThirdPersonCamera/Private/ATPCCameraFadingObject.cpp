...
FATPCFadingActorInfo::FATPCFadingActorInfo(AActor* InActor, bool bInFadeOut, float InFadeTimeSpeed, float InFadeValue /*= 1.f*/)
	: Actor(InActor), bFadeOut(bInFadeOut), FadeOutTime(InFadeTimeSpeed), FadeValue(InFadeValue)
{
}
...
void UATPCCameraFadingObject::AddManualFadeActor(AActor* Actor, bool bWithCustomeTime /*= false*/, float CustomFadeOutTime /*= 0.f*/)
{
	if (Actor == nullptr)
	{
		return;
	...
	ManualFadeingActors.Add(Actor);

	float fadeOutTime = bWithCustomeTime ? CustomFadeOutTime : GetCamera().GetCurrentCameraMode()->CameraModeSettings.FadeSettings.FadeOutTime;
...
{
	ManualFadeingActors.Remove(Actor);
}

void UATPCCameraFadingObject::ClearManualFadeActors()
{
	ManualFadeingActors.Empty();
}
...
void UATPCCameraFadingObject::UpdateFadingActorsList(TArray<AActor*> RelevantActors)
{
	...
	for (auto& actor : ManualFadeingActors)
	{
