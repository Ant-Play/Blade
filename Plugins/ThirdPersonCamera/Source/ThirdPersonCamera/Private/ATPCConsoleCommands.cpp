...
class FStructFinderVisitor : public FStructVisitor
{
public:
	FStructFinderVisitor(const FString& InPathToProperty)
	{
...
private:
	void VisitSimpleProperty(FProperty* Property, const void* Value)
	{
		...
		IConsoleManager::Get().RegisterConsoleCommand(*cmdName, TEXT(""), FConsoleCommandWithWorldAndArgsDelegate::CreateStatic(K2_OnEnterCameraModePropertyConsole, pathToProperty));
	}
private:
...
void SetCameraModeConsole(const TArray<FString>& Args, UWorld* World)
{
	...
	{
		UE_LOG(LogTPC, Display, TEXT("SetCameraMode given incorrect number of argumets. Usage: %s"), SetCameraModeConsoleUsage);
		return;
	}
...
void PrintCameraModeConsole(UWorld* World)
{
	if (UATPCCameraComponent* cameraComponent = GetPlayerCameraComponentFromWorld(World))
	{
		...
		UE_LOG(LogTPC, Display, TEXT("CameraMode: %s%s %s"), *cameraComponent->GetCurrentCameraModeTag().ToString(), LINE_TERMINATOR, *structPrinterVisitor.GetStructAsString());
	}
}
...
void K2_OnEnterCameraModePropertyConsole(const TArray<FString>& Args, UWorld* World, FString PathToProperty)
{
	if (UATPCCameraComponent* cameraComponent = GetPlayerCameraComponentFromWorld(World))
	{
		FStructFinderVisitor finderStructVisitor(PathToProperty);

		FStructIterator structIterator(finderStructVisitor, FATPCCameraMode::StaticStruct(), &cameraComponent->GetCurrentCameraMode()->CameraModeSettings);
		structIterator.Iterate();

		if (finderStructVisitor.GetFoundProperty() != nullptr)
		{
			FString stringPropValue;
			void* propertyValue = const_cast<void*>(finderStructVisitor.GetFoundPropertyValue());
			if (FNumericProperty* numericProperty = CastField<FNumericProperty>(finderStructVisitor.GetFoundProperty()))
			{
				if (Args.Num() != 0)
				{
					numericProperty->SetNumericPropertyValueFromString(propertyValue, *Args[0]);
					cameraComponent->ValidateComponent(true);
				}
				else
				{
			...
			else if (FBoolProperty* boolProperty = CastField<FBoolProperty>(finderStructVisitor.GetFoundProperty()))
			{
				if (Args.Num() != 0)
				{
			...
			{
				UE_LOG(LogTPC, Warning, TEXT("%s: %s"), *finderStructVisitor.GetFoundProperty()->GetNameCPP(), *stringPropValue);
			}
		}
	}
}
...
void UATPCCameraComponent::RegisterConsoleCommands()
{
	...
	IConsoleManager::Get().RegisterConsoleCommand(TEXT("ATPC.SetCameraMode"), SetCameraModeConsoleUsage, FConsoleCommandWithWorldAndArgsDelegate::CreateStatic(SetCameraModeConsole));

	IConsoleManager::Get().RegisterConsoleCommand(TEXT("ATPC.PrintCameraMode"), SetCameraModeConsoleUsage, FConsoleCommandWithWorldDelegate::CreateStatic(PrintCameraModeConsole));
